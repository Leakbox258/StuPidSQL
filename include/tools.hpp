...
#define timmerOn                                                               \
    {                                                                          \
        ...
        std::cerr << counter << " timmer On: " << __FILE__ << ":" << __LINE__  \
                  << std::endl;                                                \
    }

#define timmerOff                                                              \
    {                                                                          \
        std::cerr << counter << " timmer Off" << __FILE__ << ":" << __LINE__   \
                  << std::endl;                                                \
        std::cerr << "timmer " << counter << ": "                              \
                  << std::difftime(std::time(nullptr), timmer) << " seconds"   \
                  << std::endl;                                                \
        timmer = 0;                                                            \
    }
...
struct is_sequential_counter<std::vector<T, Allocator>> : std::true_type {};
template <typename T, typename Allocator>
struct is_sequential_counter<std::list<T, Allocator>> : std::true_type {};

using string = std::string;

template <typename T> inline bool inRange(T &&value, T &&low, T &&high) {
    return value >= low && value <= high;
}

template <typename T, typename... Args>
inline bool inSet(T &&value, const Args &...Enum) {
...
inline Ty inSetAndMap(Tx &&value, Args &&...Enums) { /* Tx, Ty, Tx, Ty... */
    ...
    return inSetAndMap_impl<Tx, Ty>(std::forward(value), tuple,
                                    std::make_index_sequence<N / 2>{});
}

template <typename Tuple, std::size_t... I>
bool all_equal_pairs_impl(const Tuple &tuple, std::index_sequence<I...>) {
...
#define SOFTWARE_INFO                                                          \
    "StuPidSQL " VERSION " (" MODIFIED ", compiled at " COMPILE_TIME ")\n"
