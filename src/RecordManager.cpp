...
RecordManager::RecordManager(const std::string &filename, const std::vector<Column> &columns)
...
size_t RecordManager::insertRecord(const std::vector<std::string> &values) {
    ...
    for (size_t i = 0; i < columns.size(); ++i) {
        if (columns[i].type == ColumnType::INT) {
            ...
            std::string val = values[i].substr(1, values[i].size() - 2); // Remove quotes
            char buffer[256] = {0};
...
void RecordManager::updateRecord(size_t record_id, size_t col_idx, const std::string &value) {
    file.seekp(record_id * record_size + 1); // Skip flag
    for (size_t i = 0; i < col_idx; ++i) {
        file.seekp((columns[i].type == ColumnType::INT) ? sizeof(int) : 256, std::ios::cur);
    }
...
bool RecordManager::matchCondition(const std::vector<std::string> &record, size_t col_idx, const std::string &op,
    ...
    if (columns[col_idx].type == ColumnType::INT) {
        ...
        std::string cond_val = value.substr(1, value.size() - 2); // Remove quotes
        if (op == "=")
            return rec_val == cond_val;
        if (op == "<")
            return rec_val < cond_val;
