...
std::unique_ptr<Command> Parser::parse(const std::string& sql) {
    ...
    if (tokens.empty()) throw std::runtime_error("Empty command");

    if (tokens[0] == "create" && tokens[1] == "database") {
        if (tokens.size() < 3) throw std::runtime_error("Missing database name");
    ...
    if (tokens[0] == "use") {
        if (tokens.size() < 2) throw std::runtime_error("Missing database name");
        return std::make_unique<UseDatabaseCommand>(tokens[1]);
    }
    if (tokens[0] == "create" && tokens[1] == "table") {
        if (tokens.size() < 5) throw std::runtime_error("Invalid create table syntax");
        ...
        while (std::getline(iss, col_def, ',')) {
            ...
            Column col{name, (type == "int" ? ColumnType::INT : ColumnType::STRING)};
            if (primary == "primary") col.is_primary = true;
            columns.push_back(col);
        }
        return std::make_unique<CreateTableCommand>(table_name, columns);
    }
    if (tokens[0] == "drop" && tokens[1] == "table") {
        if (tokens.size() < 3) throw std::runtime_error("Missing table name");
        return std::make_unique<DropTableCommand>(tokens[2]);
    }
    if (tokens[0] == "insert") {
        if (tokens.size() < 4 || tokens[2] != "values") throw std::runtime_error("Invalid insert syntax");
    ...
    if (tokens[0] == "select") {
        if (tokens.size() < 4 || tokens[2] != "from") throw std::runtime_error("Invalid select syntax");
    ...
    if (tokens[0] == "delete") {
        if (tokens.size() < 2) throw std::runtime_error("Invalid delete syntax");
        std::string table_name = tokens[1];
        std::string where_cond;
    ...
    if (tokens[0] == "update") {
        if (tokens.size() < 5 || tokens[2] != "set") throw std::runtime_error("Invalid update syntax");
        std::string table_name = tokens[1];
        ...
        return std::make_unique<UpdateCommand>(table_name, column, value, where_cond);
    }
