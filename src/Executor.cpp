...
void Executor::execute(const Command &cmd) {
    if (auto *create_db = dynamic_cast<const CreateDatabaseCommand *>(&cmd)) {
    ...
    } else if (auto *drop_db = dynamic_cast<const DropDatabaseCommand *>(&cmd)) {
    ...
    } else if (auto *create_tbl = dynamic_cast<const CreateTableCommand *>(&cmd)) {
        if (!db_manager.getCurrentDatabase())
            throw std::runtime_error("No database selected");
        db_manager.getCurrentDatabase()->createTable(create_tbl->table_name, create_tbl->columns);
        ...
        if (!db_manager.getCurrentDatabase())
            throw std::runtime_error("No database selected");
        db_manager.getCurrentDatabase()->getTable(insert->table_name)->insert(insert->values);
    } else if (auto *select = dynamic_cast<const SelectCommand *>(&cmd)) {
        if (!db_manager.getCurrentDatabase())
            throw std::runtime_error("No database selected");
        db_manager.getCurrentDatabase()->getTable(select->table_name)->select(select->column, select->where_cond);
    } else if (auto *del = dynamic_cast<const DeleteCommand *>(&cmd)) {
        if (!db_manager.getCurrentDatabase())
            throw std::runtime_error("No database selected");
        db_manager.getCurrentDatabase()->getTable(del->table_name)->del(del->where_cond);
